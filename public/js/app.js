...
(() => {
  ...
  const RTC_CONFIG = {
    iceServers: [
      ...
      // Free TURN relay (Open Relay Project) — lets audio/video flow when
      // both users are behind strict NATs / mobile networks.
  ...
  function newPC(peerId) {
    ...
    pc.ontrack = e => {
      ...
      // Some browsers block autoplay — explicitly start playback
      rv.play().catch(() => {
        setCallState('Tap anywhere to enable sound');
        document.addEventListener('pointerdown', () => rv.play(), { once: true });
      });
    };
    pc.oniceconnectionstatechange = () => {
      if (['connected', 'completed'].includes(pc.iceConnectionState)) setCallState('Connected', true);
    };
  ...
  function mediaErrorMsg(ex, video) {
    const dev = video ? 'Camera/microphone' : 'Microphone';
    if (ex.name === 'NotAllowedError') return dev + ' is blocked — tap the 🔒 icon in the address bar → Site settings → Allow';
    if (ex.name === 'NotFoundError') return 'No ' + (video ? 'camera' : 'microphone') + ' found on this device';
    if (ex.name === 'NotReadableError') return dev + ' is being used by another app — close it and try again';
    return dev + ' error: ' + ex.name;
  }
  ...
  async function startCall(video) {
    ...
    } catch (ex) { return toast(mediaErrorMsg(ex, video)); }
  ...
  async function acceptCall() {
    ...
    try {
      localStream = await getMedia(video);
    } catch (ex) { socket.emit('call:decline', { to: from }); return endCall(false, mediaErrorMsg(ex, video)); }
    Sounds.stopRing();
